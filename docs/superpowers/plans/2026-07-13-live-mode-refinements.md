...
### Task 6: Student reveal leaderboard (added 2026-07-14)

**Files:**
- Modify: `src/components/game/StudentReveal.tsx`
- Modify: `src/components/game/StudentGameView.tsx` (one prop added at the `StudentReveal` call site)

**Interfaces:**
- Consumes: `GameLeaderboardEntry` type from `src/hooks/useGamePolling.ts`; `state.leaderboard` already present in the poll response during `reveal`/`podium` (no API change).
- Produces: `StudentReveal` gains a required `leaderboard: GameLeaderboardEntry[]` prop. External behavior otherwise unchanged (mount-keyed sound cue untouched).
...
git commit -m "feat(live): show full ranked leaderboard to students on every reveal"
```
